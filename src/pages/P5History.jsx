...
import { ArrowDownLeft, ArrowUpRight, ShoppingCart, History, ReceiptText } from 'lucide-react';
...
const SC_FLOWS = [
  { dir: 'out', label: '兑换首发权', sub: 'A×12  B×7  C×23',   amount: '−9 亿 SC',    time: '2026-05-22 16:42' },
  { dir: 'in',  label: '补贴到账',  sub: '本轮补贴 +3.7%',     amount: '+3.11 亿 SC', time: '2026-05-22 16:40' },
  { dir: 'in',  label: '兑换 SC',   sub: '10 DOS → 10 亿 SC',  amount: '+10 亿 SC',   time: '2026-05-22 14:23' },
  { dir: 'out', label: '兑换首发权', sub: 'A×2  B×1',          amount: '−13 亿 SC',   time: '2026-05-21 20:11' },
  { dir: 'in',  label: '补贴到账',  sub: '本轮折让 36%',       amount: '+5.20 亿 SC', time: '2026-05-21 20:08' },
  { dir: 'in',  label: '兑换 SC',   sub: '5 DOS → 5 亿 SC',    amount: '+5 亿 SC',    time: '2026-05-21 10:15' },
  { dir: 'out', label: '兑换首发权', sub: 'B×5  C×10',         amount: '−25 亿 SC',   time: '2026-05-20 14:05' },
  { dir: 'out', label: '兑换首发权', sub: 'A×3',               amount: '−15 亿 SC',   time: '2026-05-19 22:10' },
  { dir: 'in',  label: '补贴到账',  sub: '本轮补贴 +2.1%',     amount: '+2.88 亿 SC', time: '2026-05-19 18:33' },
  { dir: 'in',  label: '兑换 SC',   sub: '20 DOS → 20 亿 SC',  amount: '+20 亿 SC',   time: '2026-05-19 09:00' },
];
...
export default function P5History() {
  ...
  const flows = lang === 'zh' ? SC_FLOWS : [
    { dir: 'out', label: 'Early Access', sub: 'A×12  B×7  C×23', amount: `−${formatScAmount(9, lang).text}`, time: '2026-05-22 16:42' },
    { dir: 'in',  label: 'Subsidy Received', sub: `Round subsidy +3.7%`, amount: `+${formatScAmount(3.11, lang).text}`, time: '2026-05-22 16:40' },
    { dir: 'in',  label: 'SC Swap', sub: `10 DOS → ${formatScAmount(10, lang).text}`, amount: `+${formatScAmount(10, lang).text}`, time: '2026-05-22 14:23' },
    { dir: 'out', label: 'Early Access', sub: 'A×2  B×1', amount: `−${formatScAmount(13, lang).text}`, time: '2026-05-21 20:11' },
    { dir: 'in',  label: 'Subsidy Received', sub: 'Round discount 36%', amount: `+${formatScAmount(5.2, lang).text}`, time: '2026-05-21 20:08' },
    { dir: 'in',  label: 'SC Swap', sub: `5 DOS → ${formatScAmount(5, lang).text}`, amount: `+${formatScAmount(5, lang).text}`, time: '2026-05-21 10:15' },
    { dir: 'out', label: 'Early Access', sub: 'B×5  C×10', amount: `−${formatScAmount(25, lang).text}`, time: '2026-05-20 14:05' },
    { dir: 'out', label: 'Early Access', sub: 'A×3', amount: `−${formatScAmount(15, lang).text}`, time: '2026-05-19 22:10' },
    { dir: 'in',  label: 'Subsidy Received', sub: `Round subsidy +2.1%`, amount: `+${formatScAmount(2.88, lang).text}`, time: '2026-05-19 18:33' },
    { dir: 'in',  label: 'SC Swap', sub: `20 DOS → ${formatScAmount(20, lang).text}`, amount: `+${formatScAmount(20, lang).text}`, time: '2026-05-19 09:00' },
  ];
              ...
              <>
                <div className="tab-enter overflow-hidden bg-tokenCard" style={{ borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-md)' }}>
                  {flows.map((item, i) => {
                    ...
                    return (
                      <div key={i} className={`flex items-center gap-3 px-4 py-3${i < flows.length - 1 ? ' border-b border-tokenBorderSubtle' : ''}`}>
                        <div className="grid h-9 w-9 shrink-0 place-items-center rounded-full" style={{ background: isOut ? 'var(--color-danger-soft)' : 'var(--color-success-soft)' }}>
                          {isOut
                        ...
                        <div className="shrink-0 text-right">
                          <p className={`font-num text-[15px] font-semibold leading-[20px] ${isOut ? 'text-tokenDanger' : 'text-tokenSuccess'}`}>{item.amount}</p>
                ...
                <p className="mt-4 text-center text-[12px] leading-[16px] text-tokenHint">{lang === 'zh' ? '已显示全部记录' : 'All records loaded'}</p>
              </>
            )
        )}
