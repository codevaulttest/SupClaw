    ...
    <>
      ...
      <div className="px-4 pt-5 pb-8">
        ...
        <div className="mb-5 flex gap-3">
          ...
          <div className="flex-1 rounded-xl px-4 py-3" style={{ background: 'var(--color-bg-card)', boxShadow: 'var(--shadow-sm)' }}>
            <p className="text-[11px] text-tokenHint">SC 余额</p>
            <p className="mt-1 font-num text-[20px] font-semibold text-tokenPrimary">{SC_BALANCE} <span className="text-[13px] font-normal">亿</span></p>
          </div>
        </div>
        ...
        <div className="mb-4 flex items-center gap-2 rounded-xl px-4 py-3" style={{ background: 'var(--color-primary-soft)', border: '1px solid var(--color-primary-border)' }}>
          ...
          <p className="text-[13px] text-tokenPrimary font-medium">兑换汇率：1 DOS = 1 亿 SC</p>
        </div>

        {/* 输入框 */}
        <div className="mb-2">
          <label className="mb-2 block text-[13px] font-medium text-tokenSub">输入 DOS 数量</label>
          <div
          ...
          >
            ...
            <button onClick={() => setAmount(String(DOS_BALANCE))} className="text-[12px] font-semibold text-tokenPrimary">全部</button>
          </div>
        </div>
        {insufficient && <p className="mb-3 text-[12px] text-tokenDanger">DOS 余额不足（可用：{DOS_BALANCE} DOS）</p>}

        {/* 预计获得 */}
        {num > 0 && !insufficient && (
          <div className="mb-5 rounded-xl px-4 py-3" style={{ background: 'var(--color-bg-card)', boxShadow: 'var(--shadow-sm)' }}>
            <p className="text-[12px] text-tokenHint">预计获得</p>
            <p className="mt-1 font-num text-[22px] font-semibold text-tokenPrimary">{willReceive} <span className="text-[14px] font-normal">亿 SC</span></p>
          </div>
        )}
